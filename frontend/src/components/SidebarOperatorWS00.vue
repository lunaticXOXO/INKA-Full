...
export default {
    ...
    methods: {
        ...
        async fetchOperasiLayak(){
            try{
                const axios = require('axios')
                const res = await axios.get('/operasi/show_operasilayak/' + 'ws00')
                if(res.data.length == null){
                    console.log("Data kosong")
                }
        ...
        async fetchOperasiSiap(){
            try{
                const axios = require('axios')
                this.namaOperator = 'ws00'
                const res = await axios.get('/operasi/get_operasi_siap/' + 'ws00')
        ...
        async fetchMaterial(){
            try{
                const axios = require('axios')
                const res = await axios.get('/operator/get_material_byoperator/' + 'ws00')
                if(res.data == null){
                    console.log("Data kosong")
                }else{
        ...
        startOperation(){
            ...
            setTimeout(() => {
                try{
                    ...
                    const axios = require('axios')
                    const res = axios.post('/operasi/response_operasi_mulai/' + 'ws00')
        ...
        akhiriOperation(){
            ...
            setTimeout(() => {
                try{
                    const axios = require('axios')
                    const res = axios.post('/operasi/response_operasi_selesai/' + 'ws00')
                    if(res.data.status == 'berhasil'){
                        console.log(res)
                    }
                }catch(error){
