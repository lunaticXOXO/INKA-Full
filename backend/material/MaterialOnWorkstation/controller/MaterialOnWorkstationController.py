...
def GetMaterialStockOnWsByIdStock(idStock):
    conn = database.connector()
    cursor = conn.cursor()
    query = "SELECT a.id,a.merk,b.workstationCode,b.login,b.logout FROM mat_d_materialstock a JOIN mat_d_materialonws b ON b.materialStock = a.id WHERE a.id = '"+idStock+"'"
    cursor.execute(query)
    row_headers = [x[0] for x in cursor.description]
    json_data = []
    records = cursor.fetchall()
...
def AddMaterialStockOnWSByIdStock(idStock):
    ...
    query_insert = "INSERT INTO mat_d_materialonws(id,workstationCode,materialStock,login)VALUES(%s,%s,%s)"
    try:
        data = request.json
        id = data["id"]
        workstationCode = data["workstationCode"]
        login = datetime.datetime.now()
        values = (id,workstationCode,idStock,login)
